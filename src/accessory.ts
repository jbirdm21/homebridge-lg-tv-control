...
export class LGTVAccessory {
   ...
   */
  private getCharacteristic(type: string): any {
  ...
  async setTurnOffSwitch(value: CharacteristicValue): Promise<void> {
    ...
    if (on) {
      ...
      setTimeout(() => {
        this.turnOffSwitchService?.updateCharacteristic(this.platform.api.hap.Characteristic.On, false);
      }, 1000);
    }
  }
