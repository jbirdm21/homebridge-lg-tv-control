...
import { ThinQCommands } from './thinq/commands';
 ...
 */
export class LGClient extends EventEmitter {
  private webosClient: WebOSClient;
  public webosCommands: WebOSCommands;
  private thinqCommands: ThinQCommands | null = null;
  ...
  constructor(
    ...
    private readonly thinqAuth: ThinQAuth | null,
    private readonly options: LGClientOptions = {},
  ) {
    super();
    
    // Initialize WebOS client
    this.webosClient = new WebOSClient(ipAddress, options?.manualIP);
    this.webosCommands = new WebOSCommands(this.webosClient, log);
    
    // Initialize ThinQ client if available
    if (thinqAuth && deviceId) {
      this.thinqCommands = new ThinQCommands(thinqAuth, deviceId);
    }
    ...
    this.webosClient.on('state', (state: Record<string, any>) => {
   ...
   * TV Discovery
   */
  static async discoverTVs(): Promise<DiscoveredTV[]> {
    return new Promise((resolve) => {
      const discoveredTVs: Record<string, DiscoveredTV> = {};
  ...
  async powerOn(): Promise<boolean> {
    ...
    if (this.options.macAddress) {
      try {
        ...
        await wakeOnLan(this.options.macAddress);
      } catch (error) {
   ...
   * App Control
   */
  async getLaunchPoints(): Promise<any[]> {
    try {
  ...
  async enableAIRecommendation(enable: boolean): Promise<boolean> {
    if (!this.thinqCommands) {
      throw new Error('ThinQ integration not available');
    }
